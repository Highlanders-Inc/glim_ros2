...
void RvizViewer::globalmap_on_update_submaps(const std::vector<SubMap::Ptr>& submaps) {
  ...
  // Collect all frames to sync this->submaps with the full global map state.
  // This correctly handles both the incremental case (1 new submap) and the
  // bulk-load case (all previous submaps loaded at once via load_path).
  std::vector<gtsam_points::PointCloud::ConstPtr> all_frames;
  all_frames.reserve(submaps.size());
  for (const auto& submap : submaps) {
    all_frames.push_back(submap->frame);
  }

  // Invoke a submap concatenation task in the RvizViewer thread
  invoke([this, all_frames, submap_poses] {
    this->submaps = all_frames;
